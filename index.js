...
async function run() {
	try {
		...
		app.get('/addSpot', logger,  async (req, res) => {
			...
			const cursor = tourismSpotCollection.find(query).skip(page*size).limit(size)
		...
		app.get('/dataCount', async(req, res) => {
			const count = await tourismSpotCollection.estimatedDocumentCount()
			res.send({count})
		})
